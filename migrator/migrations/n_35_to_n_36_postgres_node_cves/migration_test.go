...
func (s *postgresMigrationSuite) TearDownTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableNodeCvesStmt.GormModel)
	pgtest.CleanUpDB(s.ctx, s.T(), s.pool)
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	s.NoError(legacyStore.UpsertMany(s.ctx, nodeCVEs))
	s.NoError(move(s.legacyDB, s.gormDB, s.pool, legacyStore))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableNodeCvesStmt.GormModel).Count(&count)
	s.Equal(int64(len(nodeCVEs)), count)
	for _, nodeCVE := range nodeCVEs {
		s.Equal(nodeCVE, s.get(nodeCVE.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.NodeCVE {
