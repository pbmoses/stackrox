...
func (s *postgresMigrationSuite) TestMigration() {
	...
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableServiceAccountsStmt.GormModel).Count(&count)
	s.Equal(int64(len(serviceAccounts)), count)
	for _, serviceAccount := range serviceAccounts {
		s.Equal(serviceAccount, s.get(serviceAccount.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.ServiceAccount {
