...
func (s *postgresMigrationSuite) TearDownTest() {
	...
	_ = s.gormDB.Migrator().DropTable(pkgSchema.CreateTableImageCvesStmt.GormModel)
	pgtest.CleanUpDB(s.ctx, s.T(), s.pool)
...
func (s *postgresMigrationSuite) TestMigration() {
	...
	s.NoError(legacyStore.UpsertMany(s.ctx, imageCVEs))
	s.NoError(move(s.gormDB, s.pool, legacyStore))
	var count int64
	s.gormDB.Model(pkgSchema.CreateTableImageCvesStmt.GormModel).Count(&count)
	s.Equal(int64(len(imageCVEs)), count)
	for _, imageCVE := range imageCVEs {
		s.Equal(imageCVE, s.get(imageCVE.GetId()))
	}
}

func (s *postgresMigrationSuite) get(id string) *storage.ImageCVE {
