...
import (
	...
	pgStore "github.com/stackrox/rox/migrator/migrations/n_04_to_n_05_postgres_image_components/postgres"
...
func move(gormDB *gorm.DB, postgresDB *pgxpool.Pool, legacyStore legacy.Store) error {
	...
	var err error
	walk(ctx, legacyStore, func(obj *storage.ImageComponent) error {
		imageComponents = append(imageComponents, obj)
		if len(imageComponents) == batchSize {
			if err := store.UpsertMany(ctx, imageComponents); err != nil {
				log.WriteToStderrf("failed to persist image_components to store %v", err)
				return err
			}
			imageComponents = imageComponents[:0]
		}
		return nil
	})
...
func walk(ctx context.Context, s legacy.Store, fn func(obj *storage.ImageComponent) error) error {
	return store_walk(ctx, s, fn)
}

func store_walk(ctx context.Context, s legacy.Store, fn func(obj *storage.ImageComponent) error) error {
